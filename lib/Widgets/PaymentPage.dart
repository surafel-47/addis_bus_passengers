...
class _WebViewChapaState extends State<WebViewChapa> {
    ...
    return SafeArea(
      child: Column(
        children: <Widget>[
          Expanded(
            child: InAppWebView(
              initialUrlRequest: URLRequest(url: WebUri.uri(Uri.parse(widget.chapaCheckoutUrl))),
  ...
  void handleSuccess(Uri? uri) {
    if (uri.toString().contains('checkout/test-payment-receipt/') || uri.toString().contains('checkout/payment-receipt/')) {
      Navigator.pop(context);
      // Payment successful
